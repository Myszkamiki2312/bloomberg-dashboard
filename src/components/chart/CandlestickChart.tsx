...
export default function CandlestickChart() {
  ...
  useEffect(() => {
    ...
    async function init() {
      ...
      // Guard: cleanup may have run before async import resolved
      ...
      // Apply data that arrived before async init completed
  ...
  return (
    ...
    >
      {lastBar && (
        <div className="flex items-center gap-4 px-3 py-1.5 border-b border-terminal-border text-[11px]">
          <span className="text-terminal-amber font-bold text-base">{formatPrice(lastBar.close)}</span>
          <span className={overallChange >= 0 ? 'text-terminal-green' : 'text-terminal-red'}>
            {formatPercent(overallChange)} ({TIMEFRAMES.find(t => t.days === days)?.label})
          </span>
          <span className="text-terminal-muted">O: {formatPrice(lastBar.open)}</span>
          <span className="text-terminal-muted">H: {formatPrice(lastBar.high)}</span>
          <span className="text-terminal-muted">L: {formatPrice(lastBar.low)}</span>
        </div>
      )}
      <div className="relative flex-1 min-h-[250px]">
        {isLoading && (
          <div className="absolute inset-0 flex items-center justify-center text-terminal-muted text-xs z-10">
            Ładowanie wykresu...
          </div>
        )}
